...
# VERSION: v1.7.0 (Fault-Tolerant Asset Core)
# DATE: 2026-06-29
# AUTHOR: CTO & 제대리 (Gemini)
# DESCRIPTION: 주식 원본 267라인 100% 완벽 보존 + 실시간 주가 누락 시 평단가 대체 가드 장착
# ==========================================
import os
import requests
...
# CTO님이 정정하신 정식 백엔드 모듈 경로 100% 보존
...
# ==========================================
# [CORE 1] 주식 전용 엔진 파이프라인 (오리지널 100% 복구 및 가드 강화)
# ==========================================
...
def search_stock():
    ...
    try:
        if market == "US":
            ...
            res = supabase.table("kr_stock_list").select("symbol,name").ilike("name", f"%{query}%").limit(50).execute()
    ...
    for stock in db_stocks:
        try:
            ...
            stock_data = {
                ...
                "quantity": qty, 
                "avg_price": avg_p, 
                "current_price": avg_p, # 기본값으로 평단가 세팅 (안전장치)
                "chg_percent": 0.0,
                "purchase_amount": 0, 
                "eval_amount": 0, 
                "total_profit": 0,
            ...
            if market == "US":
                # 💡 가드 장치: 서비스가 고장 나거나 스케줄러가 멈춰서 실시간 가격이 없어도 터지지 않게 방어합니다.
                try:
                    rt = UsStockService.get_realtime_price(symbol)
                    if rt and rt.get("current_price"):
                        stock_data["current_price"] = float(rt.get("current_price"))
...
def save_portfolio():
    ...
    market, symbol, name, tx_type = data.get("market", "KR"), data.get("symbol"), data.get("name"), data.get("tx_type", "BUY")
    input_qty, input_price = float(data.get("quantity", 0)), float(data.get("avg_price", 0))
    ...
    try:
        ...
        if existing.data and len(existing.data) > 0:
            ...
            if tx_type == "BUY":
                ...
                supabase.table("stock_portfolio").update({"quantity": new_qty, "avg_price": round(new_price, 2) if market=="US" else int(new_price)}).eq("symbol", symbol).execute()
            else:
                new_qty = old_qty - input_qty
            ...
            supabase.table("stock_portfolio").insert({"market": market, "symbol": symbol, "name": name, "quantity": input_qty, "avg_price": input_price}).execute()
...
def get_real_estate():
    ...
    try:
        ...
        for item in db_estates:
            ...
            if is_watch: watch_assets.append(estate_data)
            else:
                holding_assets.append(estate_data)
                total_eval_re += c_price
                total_debt_re += debt
                if item.get("holding_type") == "LEASE": net_lease_cash += (c_price - debt)

        net_worth_re = total_eval_re - total_debt_re
        return jsonify({"total_evaluation_re": total_eval_re, "total_debt_re": total_debt_re, "net_worth_re": net_worth_re, "net_lease_cash": net_lease_cash, "holding_assets": holding_assets, "watch_assets": watch_assets})
    except Exception as e:
