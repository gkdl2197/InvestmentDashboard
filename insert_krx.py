...
# DESCRIPTION: Supabase 공식 인증 규격 매핑 완료, 주식/부동산 데이터 바인딩 최종 완치본
# ==========================================
import os
import requests
...
# 💡 Vercel 프로덕션 환경에 등록된 Supabase 정식 URL과 보안 서비스 롤 키를 정확하게 매핑합니다.
SUPABASE_URL = os.getenv("SUPABASE_URL") or os.getenv("NEXT_PUBLIC_SUPABASE_URL")
# 💡 핵심 수술: Supabase가 인지하는 공식 ANON 키 또는 SERVICE_ROLE 키 규격을 다이렉트로 매핑합니다.
...
if SUPABASE_URL and SUPABASE_KEY:
    try:
        ...
        print("✅ api.py Supabase 공식 인증 엔진 연결 성공!")
    except Exception as e:
        print(f"❌ api.py Supabase 인증 결함 에러: {str(e)}")
else:
    print("⚠️ 경고: Supabase 공식 환경 변수 로드 실패")
